...
@if ($loggedIn == $post->user->id || Auth::user()->permission >= 1)
            ...
            <div class="box">
                ...
                <div style="display: flex;justify-content: space-evenly;margin: 10px">
                    <a href="{{ route('home.profile', ['id' => $post->user->id]) }}">
                        <div class="username" style="display:flex;flex-direction: column;align-items: center;">
                            {{$post->user->name}}
    ...
    <div class="box">
        <div class="buttons">
            {{-- Back --}}
            <a href="{{ route('home.show', ['id' => $post->id]) }}" class="btn btn-2">Back</a>
                
            {{-- Delete --}}
            <form method="POST" action="{{ route('home.destroy', ['id' => $post->id]) }}" class="btn btn-2" style="top:8px;">
                @csrf
                @method('DELETE')
            ...
            {{-- Submit --}}
            <button type="submit" form="postForm" class="btn btn-2">Submit</button>
        </div>
    </div>     
@endif
@endsection
