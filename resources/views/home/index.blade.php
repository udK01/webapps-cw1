...
@section('content')
    ...
    <div style="display:flex;flex-direction: column;gap: 10px;align-items: center;justify-content: center;flex-wrap:wrap;">
        @foreach ($posts as $post)
        <a href="{{ route('home.show', ['id' => $post->id]) }}">
            <div class="box">
                <div style="display: flex;justify-content: space-evenly;">
                    <div style="display:flex;flex-direction: column;align-items: center;font-size: 18px;font-weight: bold;">{{$post->user->name}}
                        <div style="font-size: 14px;font-weight: lighter;color: #E09F3E">#{{$post->user->handle->handle_name}}</div>
                    </div>
                    <div style="display:flex;flex-direction: column;align-items: center;font-size: 18px;font-weight: bold;">posted at: 
                        <div style="font-size: 14px;font-weight: lighter;color: #E09F3E">{{$post->created_at}}</div>
                    </div>
                </div>
                <div style="display:flex;justify-content:center;font-size: 16px;font-weight:lighter">{{$post->title}}</div>
                <div style="display:flex;justify-content:center;border-radius: 5px;padding: 5px; gap: 10px">
                    @foreach ($post->tags as $tag)
                        <div style="display:flex;border-radius: 5px;padding: 5px;color:#EDE3E9;border: 3px solid #540B0E;background: #540B0E">
                                {{$tag->tag}}
                        </div>
                    @endforeach
                </div>
                @if (!empty($post->image))
                    <div style="display: flex;align-items: center;justify-content: center;padding: 10px;border-top: 3px dashed #540B0E;border-bottom: 3px dashed #540B0E">
